...
export const normalizeProfileData = (response) => {
  ...
  const contactPersonDetails =
    typeof contactPersonRaw === "string"
      ? contactPersonRaw.split(": ")
      : ["", "", ""];

  // Получаем сайт компании
  const companyWebsite = Array.isArray(
    response[dictionaryProfileData.companyWebsite]
  )
  ...
  return {
    headerData: {
      ...
      [profileFields.companyDescription]:
        response[dictionaryProfileData.companyDescription] ?? "Описание",
    },
    mainInfo: {
